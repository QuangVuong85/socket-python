...
class TestValues(object):
    ...
    def map_done_cb(self, data):
        with Pool(done_callback=self.result_cb) as pool:
            pool.map(self.worker, data)

    def map_failed_cb(self, data):
        pool = Pool(failed_callback=self.failed_cb)
        pool.map(self.worker, data)
        pool.shutdown()
        self.result = sum(pool.done)
...
class TestLists(object):
    ...
    def map_done_cb(self, data):
        pool = Pool(done_callback=self.result_cb)
        pool.map(self.worker, data)
        pool.shutdown()
...
class TestCompress(object):
    ...
    def pack_compress_cb(data):
        # noinspection PyArgumentList
        return zstd.ZstdCompressor(write_content_size=True, write_checksum=True, level=14).compress(
    ...
    def pack_compress_gen_cb(data):
        # noinspection PyArgumentList
        yield zstd.ZstdCompressor(write_content_size=True, write_checksum=True, level=14).compress(
            msgpack.packb(data, use_bin_type=True))
